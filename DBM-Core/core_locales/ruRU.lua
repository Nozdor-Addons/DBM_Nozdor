...
L.HOW_TO_USE_MOD  = "Добро пожаловать в " ..
	L.DBM ..
	". Наберите /dbm help, чтобы получить список поддерживаемых команд. Для доступа к настройкам наберите /dbm в чате. Загрузите конкретные зоны вручную, чтобы настроить определённых боссов по Вашему вкусу. "
	..
	L.DBM ..
	" установит настройки по умолчанию для Вашей специализации, но Вы, возможно, захотите настроить их более тонко."
...
L.BAD_LOAD                 = L.DBM ..
	" не удалось полностью загрузить модуль для этого подземелья, т.к. Вы находитесь в режиме боя. Как только Вы выйдете из боя, пожалуйста, сделайте /console reloadui как можно скорее."
...
L.DYNAMIC_DIFFICULTY_CLUMP = L.DBM ..
	" отключил динамическое окно проверки дистанции на этом боссе, т.к. нет точной информации о необходимом количестве игроков в одном скоплении для рейда вашего размера."
L.DYNAMIC_ADD_COUNT        = L.DBM ..
	" отключил предупреждения о появлении помощников на этом боссе, т.к. нет точной информации о количестве помощников, которые появляются в рейде вашего размера."
L.DYNAMIC_MULTIPLE         = L.DBM ..
	" отключил несколько таймеров и предупреждений на этом боссе, т.к. нет точной информации о том, как работают механики энкаунтера для рейда вашего размера."

L.LOOT_SPEC_REMINDER = "Ваша текущая специализация %s. Вы выбрали добычу для специализации %s."

L.BIGWIGS_ICON_CONFLICT = L.DBM ..
	" обнаружил, что у Вас включена установка меток в BigWigs и " ..
	L.DBM .. " одновременно. Пожалуйста, отключите метки в одном из них, чтобы избежать конфликтов"
...
L.BONUS_SKIPPED = L.DBM ..
	" автоматически закрыл окно бонусного броска. Если Вам нужно вернуть это окно, введите /dbmbonusroll в течение 3 минут"
...
L.PROFILE_NOT_FOUND          = "<" ..
	L.DBM .. "> Ваш текущий профиль повреждён. " .. L.DBM .. " загрузит профиль 'По умолчанию'."
...
L.NOTESHAREERRORSOLO        = "Одиноко? Вы не должны передавать заметки самому себе" --TODO : rofl
...
L.SOUNDKIT_MIGRATION    = "Один или более из ваших звуков предупреждений/спец-предупреждений были сброшены на по умолчанию из-за несовместимого медиаформата или неверного пути. "
	.. L.DBM .. " поддерживает только звуковые файлы, находящиеся в папке с аддонами, или SoundKit ID"
...
L.VERSIONCHECK_FOOTER       = "Найдено %d |4игрок:игрока:игроков; с " ..
	L.DBM .. " и %d |4игрок:игрока:игроков; с BigWigs"
L.VERSIONCHECK_OUTDATED     = "Следующие %d игрок(и) имеют устаревшую версию: %s"
L.YOUR_VERSION_OUTDATED     = "Ваша версия " ..
	L.DEADLY_BOSS_MODS .. " устарела! Пожалуйста, посетите " ..
	L.UPDATEREMINDER_URL .. " для загрузки последней версии."
L.VOICE_PACK_OUTDATED       = "В Вашем голосовом пакете " ..
	L.DBM ..
	" отсутствуют звуки, поддерживаемые этой версией " ..
	L.DBM ..
	". Фильтр звуков спецпредупреждений был отключён. Пожалуйста, скачайте обновлённую версию голосового пакета или свяжитесь с автором для обновления, которое содержит отсутствующие звуковые файлы."
L.VOICE_MISSING             = "Выбранный Вами голосовой пакет " ..
	L.DBM ..
	" не найден. Если это ошибка, убедитесь, что Ваш голосовой пакет правильно установлен и включен в модификациях."
L.VOICE_DISABLED            = "У Вас установлен по крайней мере один голосовой пакет "
	..
	L.DBM ..
	", но ни один не включён. Если Вы собираетесь использовать голосовой пакет, убедитесь, что он выбран в 'Spoken Alerts', иначе удалите неиспользуемые голосовые пакеты, чтобы скрыть это сообщение."
...
L.UPDATEREMINDER_FOOTER_GENERIC = "Нажмите " ..
	(IsMacClient() and "Cmd-C" or "Ctrl-C") .. ", чтобы скопировать ссылку в буфер обмена."
L.UPDATEREMINDER_DISABLE        = "ПРЕДУПРЕЖДЕНИЕ: В связи с тем, что Ваш " ..
	L.DEADLY_BOSS_MODS ..
	" сильно устарел, он был отключён до обновления. Это сделано для того, чтобы старый и несовместимый код не вызывал плохой игровой опыт для Вас и других членов группы."
L.UPDATEREMINDER_HOTFIX         = "Ваша версия " ..
	L.DBM ..
	" будет иметь некорректные таймеры или предупреждения во время этого энкаунтера. Это исправлено в новой версии."
L.UPDATEREMINDER_HOTFIX_ALPHA   = "Ваша версия " ..
	L.DBM ..
	" будет иметь некорректные таймеры или предупреждения во время этого энкаунтера. Это исправлено в следующей версии (или альфа-версии)."
L.UPDATEREMINDER_MAJORPATCH     = "ПРЕДУПРЕЖДЕНИЕ: Из-за того, что Ваш " ..
	L.DEADLY_BOSS_MODS ..
	" устарел, он был отключён до обновления, т.к. это большой игровой патч. Это необходимо для того, чтобы старый и несовместимый код не приводил к ухудшению игрового опыта для Вас и членов вашей группы. Убедитесь, что вы скачали новую версию с Curse, WoWI или со страницы релизов GitHub."
L.VEM                           = "ПРЕДУПРЕЖДЕНИЕ: Вы используете " ..
	L.DEADLY_BOSS_MODS ..
	" и Voice Encounter Mods одновременно. DBM не был загружен, т.к. эти два аддона не могут работать вместе."
L.OUTDATEDPROFILES              = "ПРЕДУПРЕЖДЕНИЕ: DBM-Profiles несовместим с этой версией "
	.. L.DBM .. ". Он должен быть удалён, прежде чем DBM сможет продолжить, чтобы избежать конфликтов."
L.OUTDATEDSPELLTIMERS           = "ПРЕДУПРЕЖДЕНИЕ: DBM-SpellTimers несовместим с " ..
	L.DBM .. " и должен быть выключен для корректной работы " .. L.DBM .. "."
L.OUTDATEDRLT                   = "ПРЕДУПРЕЖДЕНИЕ: DBM-RaidLeadTools несовместим с " ..
	L.DBM .. ". DBM-RaidLeadTools больше не поддерживается и должен быть удалён для корректной работы "
	.. L.DBM .. "."
L.VICTORYSOUND                  = "ПРЕДУПРЕЖДЕНИЕ: DBM-VictorySound несовместим с этой версией "
	.. L.DBM .. ". Он должен быть удалён, чтобы " ..
	L.DBM .. " мог продолжить работу без конфликтов."
L.DPMCORE                       = "ПРЕДУПРЕЖДЕНИЕ: Deadly PvP mods несовместимы с этой версией "
	.. L.DBM .. ". Чтобы продолжить, удалите их, чтобы избежать конфликтов."
L.DBMLDB                        = "ПРЕДУПРЕЖДЕНИЕ: DBM-LDB теперь встроен в DBM-Core. Рекомендуется удалить 'DBM-LDB' из папки с Вашими аддонами."
L.DBMLOOTREMINDER               = "ПРЕДУПРЕЖДЕНИЕ: Обнаружен установленный DBM-LootReminder. Этот аддон больше не совместим с клиентом WoW Retail и приводит к поломке пулл таймеров "
	.. L.DBM .. ". Рекомендуется удалить этот аддон."
L.UPDATE_REQUIRES_RELAUNCH      = "ПРЕДУПРЕЖДЕНИЕ: Это обновление " ..
	L.DBM ..
	" не будет работать корректно, если Вы не перезапустите игровой клиент. Это обновление содержит новые файлы или изменения в .toc файле, которые не могут быть загружены через ReloadUI. Вы можете столкнуться со сломанной функциональностью или ошибками, если продолжите без перезапуска клиента."
L.OUT_OF_DATE_NAG               = "Ваша версия " ..
	L.DBM ..
	" устарела и этот энкаунтер имеет новые фичи и багфиксы в новой версии. Рекомендуется обновиться, чтобы не было отсутствующих важных предупреждений, или таймеров, или крика от Вас, на который рассчитывает остальная группа."

L.MOVABLE_BAR = "Перетащите!"

L.PIZZA_SYNC_INFO      = "|Hplayer:%1$s|h[%1$s]|h транслирует Вам таймер " ..
	L.DBM ..
...
L.RANGECHECK_OPTION_AVERAGE    = "Средняя 0.05 сек"
L.RANGECHECK_OPTION_FAST       = "Быстрая 0.01 сек (real-time)"
...
L.AUTO_ICONS_OPTION_TARGETS_ROSTER   = "Устанавливать метки на цели $spell:%s с приоритетом списка рейда"
...
L.AUTO_ARROW_OPTION_TEXT2            = "Показывать стрелку " ..
	L.DBM .. " от цели, на которой $spell:%s"
L.AUTO_ARROW_OPTION_TEXT3            = "Показывать стрелку " ..
	L.DBM .. " к определённому месту для $spell:%s"
