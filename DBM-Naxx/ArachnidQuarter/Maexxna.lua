...
local timerWebSpray		= mod:NewNextTimer(sprayCD, 29484, nil, nil, nil, 2)
local timerWebWrap		= mod:NewNextTimer(39.6, 28622, nil, "RangedDps|Healer", nil, 3)
...
local function SprayLoop(self)
	warnWebSprayNow:Show()
	warnWebSpraySoon:Cancel()
	warnWebSpraySoon:Schedule(sprayCD - 5)
	timerWebSpray:Start(sprayCD)
	self:Unschedule(SprayLoop)
	self:Schedule(sprayCD, SprayLoop, self)
end

function mod:OnCombatStart(delay)
	local first = sprayCD - delay
	warnWebSpraySoon:Schedule(first - 5)
	timerWebSpray:Start(first)
	self:Schedule(first, SprayLoop, self)

	timerWebWrap:Start(20.1 - delay)
...
function mod:SPELL_CAST_SUCCESS(args)
	if args.spellId == 29484 or args.spellId == 54124 or args.spellId == 54125 then
		self:Unschedule(SprayLoop)
		SprayLoop(self)
	end
end
