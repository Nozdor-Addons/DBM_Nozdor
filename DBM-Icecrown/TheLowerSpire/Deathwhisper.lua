...
function mod:SPELL_CAST_SUCCESS(args)
    ...
    -- Vengeful Shade fixate (пример, надо отловить по combatlog)
    elseif self:GetCIDFromGUID(args.sourceGUID) == 38222 then
        ...
        if targetName then
            if targetName == UnitName("player") then
                ...
                warnVengefulShade:Show(targetName)
            end
        end
    end
end
...
function mod:COMBAT_LOG_EVENT_UNFILTERED(...)
    ...
    if eventType == "SPELL_CAST_SUCCESS" and srcGUID and self:GetCIDFromGUID(srcGUID) == 86397 then
        print("Shade cast:", spellId, spellName, "->", dstName)
    end
end