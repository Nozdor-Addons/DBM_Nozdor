...
if DBM.NewerVersion then
	frameRevision:SetText(L.DBMNozdor.. " " .. DBM.DisplayVersion.. " (" .. DBM:ShowRealDate(DBM.Revision) .. "). |cffff0000Version " .. DBM.NewerVersion .. " is available.|r")
else
	frameRevision:SetText(L.DBMNozdor.. " " .. DBM.DisplayVersion.. " (" .. DBM:ShowRealDate(DBM.Revision) .. ")")
end
...
do
	...
	frameHeaderButton:SetScript("OnMouseUp", function()
		...
		DBM:ShowUpdateReminder(nil, nil, CL.COPY_URL_DIALOG, DBM.DisplayVersion.. " (" .. DBM:ShowRealDate(DBM.Revision) .. ")") -- для простого копирования версии (вдруг кто прочтет)
	end)
end
...
frameWebsiteButtonA:SetScript("OnMouseUp", function()
	DBM:ShowUpdateReminder(nil, nil, CL.COPY_URL_DIALOG, "https://discord.gg/bfcYWgDHt8")
end)
